...
export default function WeekMenu({ recipes }: { recipes: Recipe[] }) {
  ...
  const fetchWeek = useCallback(async () => {
    setLoading(true);
    try { const r = await fetch(`/api/menu?week_start=${weekStart}`); if (r.ok) setWeekData(await r.json()); }
  ...
  async function addDish(meal: string, recipe: Recipe) {
    await fetch("/api/menu/dishes", { method: "POST", headers: { "Content-Type": "application/json" },
  ...
  function startAdding(meal: string) {
    ...
    setTimeout(() => { searchRef.current?.focus(); searchRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }); }, 80);
  }
  ...
  const searchResults = search.trim().length > 0
    ...
    : [];
  const daysWithDinner = new Set(weekData.slots.filter(s => s.meal_type === "dinner" && s.dishes.length > 0).map(s => s.day_date));
    ...
    <div className="min-h-screen bg-[var(--mk-cream)]">
      ...
      <div style={{ background: "linear-gradient(135deg, #3E7B5A 0%, #6AAF88 100%)" }} className="px-5 pt-10 pb-5">
        <div className="flex items-center gap-2 mb-4">
          <RingsLogo />
          <span className="text-white font-bold text-base tracking-wide">Memory Kitchen</span>
        </div>
        <div className="relative">
          ...
          {search.trim().length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl overflow-hidden z-30" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.15)" }}>
              ...
              {searchResults.length === 0 ? <p className="px-4 py-3 text-xs text-neutral-400">No recipes found</p>
                : searchResults.map(r => (
      ...
      <div className="px-5 pt-4 pb-28">
        ...
        {loading ? <p className="text-xs text-neutral-400 text-center py-10">Loading…</p> : (
          <div className="space-y-2">
            ...
            ) : <button onClick={() => startAdding("breakfast")} className="text-xs font-medium py-1" style={{ color: "var(--mk-terracotta)" }}>+ Add breakfast</button>}
            ...
            ) : <button onClick={() => startAdding("lunch")} className="text-xs font-medium py-1" style={{ color: "var(--mk-terracotta)" }}>+ Add lunch</button>}
          </div>
        )}
        {/* Nestor */}
